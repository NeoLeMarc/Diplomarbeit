...
In dieser Arbeit wird ein Sensornetzwerk zur Patientenüberwachung entworfen. Dieses System besteht aus einer Kombination
...
\begin{itemize}
    \item{Erste-Hilfe-Sensor:} Der eigentliche Sensor, der zur Überwachung der Patienten eingesetzt wird. Dieser 
    ...
    \item{\emph{MANVConnector}}: Der Connector fungiert als Adapter, der eine Übersetzung zwischen der 
                                 ...
                                 vornimmt. Vereinfacht handelt es sich um einen Treiber für den 
    ...
    \item{\emph{MANVServer}}: Der \emph{MANVServer} verwaltet alle Informationen über das Sensornetzwerk. Hierzu
    ...
    \item{\emph{Client-Anwendungen}}: Die Clientanwendungen, also \emph{MANVGui} und \emph{MANVMobileGui} bilden
                                      die Benutzerschnittstelle des Sensornetzwerkes. Diese wurden ebenfalls
...
Low-Dropout-Spannungsregler des Typs \emph{LP2980-3.3} der Firma \emph{National Semiconductor}. Dieser liefert eine stabile
...
verwendet werden.  Zusätzlich besthet die Möglichkeit, auf die serielle Schnittstelle des \emph{ZigBit}-Moduls 
...
möglich\footnote{Einige Werkzeuge bieten die Möglichkeit, eine Consolenverbindung
über den \emph{UART} des \emph{ADuC-7026-Mikrocontrollers} zu öffnen, so direkt mit dem \emph{ZigBit}-Modul 
Kommunikation möglich ist.}.  Ein Zugriff über \emph{JTAG} ist nicht vorgesehen.
...
\subsubsection{\emph{MANV-USB-Stick}}
\Abbildungps{!htb}{0.25}{diagramme/Ansteuerung-USB-Stick.pdf}{label}{Datenfluss zwischen Server und Netzwerk}

Der \emph{MANV-USB-Stick} ist die Schnittstelle zwischen Sensornetz und Computer. Es handelt sich um einen 
...
Der \emph{MANVConnector} hat einerseits die Aufgabe, Daten die von dem \emph{MANV-USB-Connector} empfangen wurden in 
...
Der \emph{MANVConnector} verfügt neben dem Hauptthread über drei weitere Threads:

\begin{itemize}
    \item{SocketWriter:} Dieser Thread entnimmt Befehle aus der \emph{CommandQueue} und sendet
                         diese über den \emph{MANV-USB-Stick} an das Sensornetz. Nun blockiert der
    ...
    \item{SocketReader:} Dieser Thread empfängt Daten aus dem Sensornetz. Handelt es sich um ein
                         ...
                         Ergebnis zur Abholung durch den \emph{SocketWriter} zur Verfügung
                         gestellt. Handelt es sich hingegen um einen Ereignis, so wird dieses
                         in die \emph{EventQueue} eingefügt.
    \item{CorbaSender:} Dieser Thread ist für die Kommunikation mit \emph{MANVServer} zuständig.
                         ...
                         in Corba-Events übersetzt und an den \emph{MANVServer} zugestellt.
\end{itemize}                          
...
Zunächst erzeugt der Hauptthread (\emph{MANVConnector.main}) die drei Warteschlangen. Dannach werden die beiden 
\emph{Threads} \emph{SocketWriter} und \emph{SocketReader} erstellt. Dannach ruft der 
\emph{Hauptthread} die \emph{CORBA}-Initialisierungsroutine auf und startet den \emph{CorbaSender}-Thread.\\
