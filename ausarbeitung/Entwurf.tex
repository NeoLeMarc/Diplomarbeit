...
\section{MANVNode}\label{entwurf_manvnode}
\Abbildungps{!htbp}{1}{diagramme/schaltplan_manvnode_aduc-crop.pdf}{schaltplan_manvnode_aduc}{Schaltplan der \emph{MANVNode}: Mikrocontroller.}
\Abbildungps{!htb}{1}{diagramme/schaltplan_manvnode_zigbit-crop.pdf}{schaltplan_manvnode_zigbit}{Schaltplan der \emph{MANVNode}: \emph{ZigBit}-Modul.}
...
Die Platine dient zur Simulation des \emph{Erste-Hilfe-Sensors}. Die Beschaltung dieser Bauteile findet sich in 
Abbildung~\ref{schaltplan_manvnode_aduc} sowie Abbildung~\ref{schaltplan_manvnode_zigbit}.
An Stelle einer echten Messung generiert die 
Platine Messwerte mit Hilfe eines Zufallszahlen-Generators. Mit diesem Generator können folgende Zustände generiert werden:
...
Die \emph{MANVNode} wird als Platine in \emph{SMD-Bausweise} realisiert. Das Layout dieser Platine wurde mit Hilfe des 
Programms \emph{Cadsoft Eagle} entworfen. In Abbildung~\ref{abbildung_manvnode} ist der Entwurf dieser Platine zu sehen.
Im Folgenden werden die einzelnen Aspekte des Entwurfs erläutert.

\subsubsection{Spannungsversorgung}
\Abbildungps{!htb}{0.75}{diagramme/schaltplan_manvnode_power-crop.pdf}{schaltplan_manvnode_power}{Schaltplan der Spannungsversorgung der \emph{MANVNode}.}
...
Um die Platine möglichst vielseitig einsetzen zu können, wurde Wert auf eine möglichst flexible Spannungsversorgung gelegt. 
Diese
...
aus dem Stromsparmodus auftreten können, werden so zuverlässig abgefangen. Der Schaltplan der Spannungsversorgung der
\emph{MANVNode} findet sich in Abbildung~\ref{schaltplan_manvnode_power}. 
...
\subsubsection{JTAG}
\Abbildungps{!htb}{0.5}{diagramme/schaltplan_manvnode_jtag-crop.pdf}{schaltplan_manvnode_jtag}{Schaltplan der \emph{MANVNode}: \emph{JTAG}.}
Die \emph{MANVNode}-Platine verfügt neben der seriellen auch über eine JTAG-Schnittstelle. Die Beschaltung dieser ist in
Abbildung~\ref{schaltplan_manvnode_jtag} zu sehen. Diese ermöglich einen Zugriff mit
...
\subsubsection{LED-Anzeige}
\Abbildungps{!htb}{1}{diagramme/schaltplan_manvnode_interface-crop.pdf}{schaltplan_manvnode_interface}{Schaltplan der \emph{MANVNode}: LED-Anzeige und Taster.}
Die \emph{MANVNode}-Platine verfügt insgesamt über 6 LEDs, die zur Anzeige des aktuellen Zustands sowie zur Fehlerdiagnose dienen. Die Beschaltung dieser LEDs findet sich in Abbildung~\ref{schaltplan_manvnode_interface}.
\begin{itemize}
...
\Abbildungps{!htb}{0.7}{diagramme/Flussdiagramm-Firmware-Mainloop.pdf}{zigbit_mainloop}{Ablauf der Ansteuerung des \emph{ZigBit}-Moduls in der \emph{ZigBit-Firmware}.}
Das \emph{ZigBit}-Modul ist via \emph{UART} mit der \emph{MANVNode} verbunden und wird über das \emph{SerialNet}-Protokoll
angesprochen. Der Ablauf ist hierbei wie folgt: 
...
\section{\emph{ZigBee-USB-Stick}}\label{usb_stick}
\Abbildungps{!htb}{0.4}{diagramme/schaltplan_usbstick.pdf}{schaltplan_usbstick}{Schaltplan des \emph{USB-Sticks}.}
...
\emph{USB}-Schnittstelle auf die 3~V des \emph{ZigBit}-Moduls umzusetzen. Der Platinenentwurf des \emph{USB}-Sticks 
ist in Abbildung~\ref{platine_usbstick} abgebildet.\\

In Verbindung mit einer Stromquelle wie z.B.
einer Batterie oder eines \emph{USB}-Netzteil dient der \emph{ZigBee-USB-Stick} gleichzeitig auch als \emph{ZigBee}-Router.
Der Betriebsmodus kann hierzu einfach mit Hilfe des Befehls \emph{AT+WROLE=1} auf den Routerbetrieb umgestellt werden.

\section{MANVSuite}
\nomenclature{UML}{\emph{Unified Modeling Language\,--\,Graphische Modelierungssprache zum Entwurf von Software.}}
\Abbildungps{!htb}{0.25}{diagramme/Ansteuerung-USB-Stick.pdf}{datenfluss_manvsuite}{Datenfluss zwischen den einzelnen Kompnenten des Gesamtsystems.}

In der vorliegenden Diplomarbeit wird der \emph{Java}-Treiber 
(\emph{MANVConnector}) entworfen und implemtiert, der die Schnittstelle zwischen \emph{MANV-Suite} und 
\emph{ZigBee-USB-Stick} bildet. Entwurf und Implementierung der \emph{MANVSuite} selbst erfolgt in \cite{Jan}. 
Abbildung~\ref{datenfluss_manvsuite} beschreibt den Datenfluss zwischen \emph{MANVSuite},  \emph{MANVConnectors} und \emph{MANVNode}:
Der \emph{MANVServer} ist Teil der \emph{MANVSuite}. Er dient als Vermittlungsstelle zwischen den einzelnen 
Softwarekomponenten, die mittels \emph{Corba} an diesen angebunden sind. Eine dieser Komponenten ist der \emph{MANVConnector},
der in Abschnitt~\ref{MANVConnector} genauer beschrieben wird. Dieser greift mittels einer seriellen Schnittstelle 
(in der Abbildung als \emph{TTY-Verbindung} bezeichnet) auf den in Abschnitt~\ref{usb_stick} beschriebene
\emph{ZigBee-USB-Stick} zu, welcher wiederum die Verbindung zu dem \emph{ZigBee-Netzwerk herstellt}. Auf diese Weise kommt die
Verbindung zwischen \emph{MANVSuite} und \emph{MANVNode} zu Stande.

\section{MANVConnector}\label{MANVConnector}
...
zuständigen \emph{MANVNodes} weiter. Das vollständige \emph{UML}-Klassendiagramm findet sich in 
Abbildung~\ref{klassendiagramm_manvconnector}.

\subsection{Threads}
Wie aus Abbildung~\ref{klassendiagramm_manvconnector_threads} zu entnehmen ist, verfügt der \emph{MANVConnector} 
neben dem Hauptthread über drei weitere \emph{Threads}: 
...
\subsection{Repräsentation eines \emph{ZigBit}-Moduls}
...
Abbildung~\ref{klassendiagramm_zigbit} zeigt die Klassenrepräsentation der \emph{ZigBit}-Module. 
Da es nicht immer möglich ist, Daten an einen \emph{ZigBit}-Knoten zu senden (dies ist z.B. dann der Fall,
wenn es sich um einen Router handelt), gibt
es zwei Arten von \emph{ZigBit}-Knoten: \emph{ZigBit} und \emph{readonlyZigBit}. Um diese trotzdem
generisch ansprechen zu können, implementieren beide das Interface \emph{iZigBit}. Eine genauere Beschreibung der
Attribute und Methoden der \emph{ZigBit}-Klassen findet sich in Abschnitt~\ref{anhang_zigbit}.


\subsection{Synchronisierung der Befehlsübertragung}
...
Dieser Sachverhalt ist auch nochmal in Abbildung~\ref{sequenzdiagramm_manvconnector} dargestellt.\\
Die vom \emph{SocketReader} gelesenen Ergebnisse und Ereignisse werden selbst wieder durch Objekte repräsentiert.

\subsection{Ergebnisse und Ereignisse}
...
Wie aus Abbildung~\ref{klassendiagramm_events} zu entnehmen ist, wird zwischen folgenden Ergebnissen und Ereignissen 
unterschieden:

\begin{itemize}
    \item{MANVEvent:} Allgemeines Event.
    \item{MANVChildJoined:} Ein neuer Sensor hat das Netzwerk betreten.
    \item{MANVChildLost:} Ein Sensor hat die Verbindung zum Netzwerk verloren.
    \item{MANVDataReceived:} Es wurden Daten von einem Sensor empfangen.
    \item{MANVStatusMessage:} Es wurde eine Satusnachricht von einem Sensor empfangen. Diese beinhaltet neben dem
                              Alarmzustand Werte für Puls- und Atemfrequenz.
    \item{MANVResult:} Allgemeines Ergebnis eines gesendeten Befehls.
    \item{MANVChildrenList:} Enthält eine Liste mit Kindknoten als Ergebnis auf den Befehl \emph{AT+WCHILDREN}.
    \item{MANVGsn:} Enthält die \emph{GSN} (Hardwareadresse) eines entfernten Knotens.
\end{itemize}

Eine genauere Beschreibung der Attribute und Methoden dieser Klassen erfolgt in Abschnitt~\ref{anhang_events}. 
\Abbildungps{!htb}{1.20}{uml_diagramme/manvconnector_uml_links.pdf}{klassendiagramm_manvconnector}{Klassendiagramm des \emph{MANV-Connectors} (Linke Seite)}
\Abbildungps{!htb}{1.20}{uml_diagramme/manvconnector_uml_rechts.pdf}{klassendiagramm_manvconnector_2}{Klassendiagramm des \emph{MANV-Connectors} (Rechte Seite)}
